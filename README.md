...
Если Avito вернул `403` на `/autoload/v2/profile`, значит приложению выдали отчеты Autoload, но не выдали управление профилем автозагрузки. В этом режиме SEB0G1SHOPCHIK все равно готовит товар и фид, а в карточке товара показывает feed URL, который нужно один раз вручную указать в кабинете Avito в разделе Автозагрузка. После этого доступные методы upload/reports продолжат работать по фактическим правам аккаунта.
